...
impl LlmClient for OllamaClient {
    async fn parse_meal(&self, text: &str) -> Result<ParsedMeal, AppError> {
        ...
        let parsed: ChatResponse = resp
            .json()
            .await
            .map_err(|e| AppError::Internal(format!("LLM response parse: {e}")))?;
        ...
        serde_json::from_str::<ParsedMeal>(&content)
            .map_err(|e| AppError::Internal(format!("LLM JSON schema violation: {e}; body={content}")))
    }
}
